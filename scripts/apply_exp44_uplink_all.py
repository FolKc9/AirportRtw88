...
# EXP44 safe uplink build.
# Preserve EXP40 AirPort lifecycle exactly: POWER handlers, enable/disable,
# link-state notifications, medium dictionary and TX queue resume behavior.
# Only change the data path after the native interface is already operational.

# 1) Release the temporary 6 Mbps bootstrap after secure association.
...
# 2) Pull-model safety: check resources before dequeue, but preserve the EXP40
# queue-resume path and all AirPort interface lifecycle behavior.
...
# - setupMediumDict()
# - enable()/disable()
# - notifyAirportLinkIdle/Up/Down()
# - APPLE80211_IOC_POWER
# - IO80211 link state

# 3) Restore true host-deferred A-MPDU doorbell batching only for aggregate-
# eligible data. Management, EAPOL and pre-BA traffic remain immediate.
...
if "EXP44 host-deferred AMPDU kick" not in s:
    ...
    s = s.replace(anchor, anchor + "\n#ifdef RTW88_MACOS\n\t/* EXP44 host-deferred AMPDU kick: management/EAPOL/pre-BA stays immediate. */\n\texp44_defer_kick = pkt_info.ampdu_en;\n#endif", 1)
...
# 4) An already-established link restored after channel changes stays under
# firmware RA instead of being pinned back to legacy 6 Mbps.
...
replace_once(
    ...
    "    /* EXP44: preserve automatic firmware RA on an already-established link. */\n    rtwdev->dm_info.fix_rate = 0xff;",
    "keep firmware RA after channel restore",
)

# Version marker only. Do not change AirPort capabilities or medium reporting.
plist = root / "AirportRtw88Legacy.kext/Contents/Info.plist"
s = plist.read_text()
s = s.replace("1.4.10-exp40", "1.5.1-exp44-uplink-safe", 1)
s = s.replace("1.4.10", "1.5.1", 1)
plist.write_text(s)
print("version marker: 1.5.1-exp44-uplink-safe")
