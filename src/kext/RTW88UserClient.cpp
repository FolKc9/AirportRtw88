...
const IOExternalMethodDispatch RTW88UserClient::sMethods[kRTW88NumSelectors] = {
    ...
    { (IOExternalMethodAction)&RTW88UserClient::sPowerOff,
      0, 0, 0, 0 },
    /* kRTW88GetPerf: appended selector; existing Starskiff ABI remains intact */
    { (IOExternalMethodAction)&RTW88UserClient::sGetPerf,
      0, 0, 0, sizeof(RTW88PerfResult) },
    /* exp8 diagnostic forced TX rate: one scalar input, 0xff=auto */
    { (IOExternalMethodAction)&RTW88UserClient::sSetTxRate,
      1, 0, 0, 0 },
};
                                     ...
                                     IOExternalMethodArguments *args)
{
    ...
    if (!args || !args->structureInput ||
        args->structureInputSize != sizeof(RTW88ConnectArgs))
        return kIOReturnBadArgument;

    /* exp22 security: bound user-controlled fixed-size strings before any
     * downstream C-string operation. */
    const RTW88ConnectArgs *ca = (const RTW88ConnectArgs *)args->structureInput;
    char safeSsid[33] = {};
    char safePassword[64] = {};
    memcpy(safeSsid, ca->ssid, 32);
    memcpy(safePassword, ca->password, 63);
    safeSsid[32] = '\0';
    safePassword[63] = '\0';
    if (safeSsid[0] == '\0')
        return kIOReturnBadArgument;
    return uc->_provider->get80211()->cmdConnect(safeSsid, safePassword);
}
...
IOReturn RTW88UserClient::sGetPerf(RTW88UserClient *uc, void *ref,
                                    IOExternalMethodArguments *args)
{
    if (!args->structureOutput || args->structureOutputSize < sizeof(RTW88PerfResult))
        return kIOReturnBadArgument;
    if (!uc->_provider || !uc->_provider->get80211())
        return kIOReturnOffline;

    RTW88PerfResult *result = (RTW88PerfResult *)args->structureOutput;
    memset(result, 0, sizeof(*result));
    IOReturn ret = uc->_provider->get80211()->cmdGetPerf(result);
    if (ret == kIOReturnSuccess)
        args->structureOutputSize = sizeof(*result);
    return ret;
}

IOReturn RTW88UserClient::sSetTxRate(RTW88UserClient *uc, void *ref,
                                      IOExternalMethodArguments *args)
{
    if (!uc || !uc->_provider || !args || args->scalarInputCount < 1)
        return kIOReturnBadArgument;
    RTW88IEEE80211 *wifi = uc->_provider->get80211();
    if (!wifi) return kIOReturnNotReady;
    return wifi->cmdSetTxRate((uint8_t)args->scalarInput[0]);
}
