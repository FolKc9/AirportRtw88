...
static void aes128_encrypt_block(const uint8_t key[16], const uint8_t in[16], uint8_t out[16])
{
    ...
    aes_sub_bytes(s); aes_shift_rows(s); aes_add_round_key(s, rk + 160);
    memcpy(out, s, 16);
}

/* CCMP is AES-CCM with an 8-byte tag and L=2. */
static void ccmp_encrypt_80211(const uint8_t key[16], const uint8_t nonce[13],
                               const uint8_t *aad, uint16_t aad_len,
                               uint8_t *data, uint16_t data_len, uint8_t mic[8])
{
    uint8_t x[16] = {}, block[16] = {}, stream[16];
    block[0] = 0x59;
    memcpy(block + 1, nonce, 13);
    block[14] = (uint8_t)(data_len >> 8); block[15] = (uint8_t)data_len;
    aes128_encrypt_block(key, block, x);

    memset(block, 0, sizeof(block));
    block[0] = (uint8_t)(aad_len >> 8); block[1] = (uint8_t)aad_len;
    uint16_t first = aad_len > 14 ? 14 : aad_len;
    memcpy(block + 2, aad, first);
    for (int i = 0; i < 16; i++) block[i] ^= x[i];
    aes128_encrypt_block(key, block, x);
    for (uint16_t off = first; off < aad_len; off += 16) {
        memset(block, 0, sizeof(block));
        uint16_t n = (uint16_t)((aad_len - off) > 16 ? 16 : (aad_len - off));
        memcpy(block, aad + off, n);
        for (int i = 0; i < 16; i++) block[i] ^= x[i];
        aes128_encrypt_block(key, block, x);
    }
    for (uint16_t off = 0; off < data_len; off += 16) {
        memset(block, 0, sizeof(block));
        uint16_t n = (uint16_t)((data_len - off) > 16 ? 16 : (data_len - off));
    ...
    memset(block, 0, sizeof(block));
    block[0] = 0x01; memcpy(block + 1, nonce, 13);
    aes128_encrypt_block(key, block, stream);
    for (int i = 0; i < 8; i++) mic[i] = x[i] ^ stream[i];
    for (uint16_t off = 0, counter = 1; off < data_len; off += 16, counter++) {
        block[14] = (uint8_t)(counter >> 8); block[15] = (uint8_t)counter;
        aes128_encrypt_block(key, block, stream);
        uint16_t n = (uint16_t)((data_len - off) > 16 ? 16 : (data_len - off));
        for (uint16_t i = 0; i < n; i++) data[off + i] ^= stream[i];
    }
}

/* Authenticate and decrypt one CCMP payload in place.  On authentication
 * failure the CTR pass is repeated, restoring the original ciphertext so the
 * caller can safely test the alternate RX layout (with/without appended FCS). */
static bool ccmp_decrypt_80211(const uint8_t key[16], const uint8_t nonce[13],
                               const uint8_t *aad, uint16_t aad_len,
                               uint8_t *data, uint16_t data_len,
                               const uint8_t mic[8])
{
    uint8_t block[16] = {}, stream[16], received_tag[8];
    block[0] = 0x01;
    memcpy(block + 1, nonce, 13);
    aes128_encrypt_block(key, block, stream); /* S0 */
    for (int i = 0; i < 8; i++)
        received_tag[i] = mic[i] ^ stream[i];

    /* CTR is symmetric: ciphertext -> plaintext. */
    for (uint16_t off = 0, counter = 1; off < data_len; off += 16, counter++) {
    ...
    /* Recreate CBC-MAC over B0, AAD, and the recovered plaintext. */
    uint8_t x[16] = {};
    memset(block, 0, sizeof(block));
    block[0] = 0x59;
    memcpy(block + 1, nonce, 13);
    block[14] = (uint8_t)(data_len >> 8);
    block[15] = (uint8_t)data_len;
    aes128_encrypt_block(key, block, x);
    ...
    uint8_t diff = 0;
    for (int i = 0; i < 8; i++)
        diff |= x[i] ^ received_tag[i];
    if (diff == 0)
        return true;

    /* Restore ciphertext after a failed MIC check. */
    memset(block, 0, sizeof(block));
    block[0] = 0x01;
    memcpy(block + 1, nonce, 13);
    for (uint16_t off = 0, counter = 1; off < data_len; off += 16, counter++) {
...
/* RFC 3610 packet vector #1 (AES-CCM, M=8, L=2).  Running this tiny
 * deterministic test before the radio starts catches any compiler/porting
 * regression in both the encrypt and decrypt paths without touching hardware. */
static bool ccmp_selftest()
{
    ...
    for (uint8_t i = 0; i < sizeof(data); i++)
        data[i] = original[i] = (uint8_t)(i + 8);
    ccmp_encrypt_80211(key, nonce, aad, sizeof(aad), data, sizeof(data), mic);
    if (memcmp(data, expected, sizeof(data)) != 0 ||
        memcmp(mic, expected + sizeof(data), sizeof(mic)) != 0)
        return false;
    if (!ccmp_decrypt_80211(key, nonce, aad, sizeof(aad), data,
                            sizeof(data), mic))
        return false;
...
static bool rtw88_consttime_equal(const uint8_t *a, const uint8_t *b, size_t len)
{
    if (!a || !b) return false;
    uint8_t diff = 0;
...
static int rtw88_replay_cmp(const uint8_t a[8], const uint8_t b[8])
{
    for (int i = 0; i < 8; i++) {
        if (a[i] < b[i]) return -1;
        if (a[i] > b[i]) return 1;
                         ...
                         uint32_t eapol_len)
{
    ...
    return rtw88_consttime_equal(rx_mic, mic, sizeof(rx_mic));
}
...
bool RTW88IEEE80211::init(RTW88PCIDevice *dev, struct pci_dev *pci)
{
    ...
    if (!ccmp_selftest()) {
        IOLog("rtw88: AES-CCMP self-test failed; refusing to start radio\n");
        return false;
    }
    _parent = dev;
    _pcidev = pci;
...
{
    if (_ptkHardwareInstalled && _powered && _hw && _hw->ops &&
        _hw->ops->set_key && _ptkConf)
        _hw->ops->set_key(_hw, DISABLE_KEY, _vif, _sta, _ptkConf);
    _ptkHardwareInstalled = false;
    ...
    memset(_ccmpRxHighestPtk, 0, sizeof(_ccmpRxHighestPtk));
    memset(_ccmpRxBitmapPtk, 0, sizeof(_ccmpRxBitmapPtk));
    memset(_ccmpRxHighestGtk, 0, sizeof(_ccmpRxHighestGtk));
    memset(_ccmpRxBitmapGtk, 0, sizeof(_ccmpRxBitmapGtk));
    _ccmpRxValidPtk = 0;
    _ccmpRxValidGtk = 0;
    _eapolReplayValid = false;
    _eapolM1Seen = false;
    _ptkInstalledThisHandshake = false;
...
void RTW88IEEE80211::releaseSta()
{
    ...
    /* exp5: stop exposing the peer to asynchronous rtw88 iterators before
     * sta_remove/free, preventing a stale station pointer during teardown. */
                                ...
                                const uint8_t *tk, uint8_t tk_len)
{
    ...
    if (*slot) {
        if (pairwise && _ptkHardwareInstalled && _hw && _hw->ops &&
            _hw->ops->set_key) {
            _hw->ops->set_key(_hw, DISABLE_KEY, _vif, _sta, *slot);
            _ptkHardwareInstalled = false;
        }
    ...
    if (pairwise) {
        if (!_hw || !_hw->ops || !_hw->ops->set_key ||
            _hw->ops->set_key(_hw, SET_KEY, _vif, _sta, key) != 0) {
            IOLog("rtw88: failed to install pairwise CCMP key for hardware TX\n");
            IOFree(key, sizeof(*key));
            return false;
        }
        _ptkHardwareInstalled = true;
    } else {
        /* Group RX remains in software; this is the path proven by exp20. */
        key->hw_key_idx = keyidx;
    }
    *slot = key;
    if (pairwise)
        memset(_ccmpTxPn, 0, sizeof(_ccmpTxPn));
    IOLog("rtw88: installed hybrid %s %s key idx=%u hw_idx=%u\n",
...
IOReturn RTW88IEEE80211::start()
{
    ...
    /* Cache everything exposed by cmdGetState while the freshly-probed
     * Linux-side objects are known to be alive.  cmdGetState is reachable
     * both from rtw88ctl and from the native Airport bridge; reading rtw_dev
     * there raced teardown and caused a page fault/use-after-free panic. */
    rtw88_get_fw_version(_rtwdev, &_fwVersion, &_fwSubVersion);
    rtw88_get_chip_name(_rtwdev, _chipName, sizeof(_chipName));
    _scanOffloadSupported =
        (_hw && _hw->ops && _hw->ops->hw_scan &&
         rtw88_hw_scan_supported(_hw));
...
{
    /* exp10: this is a hot path (tens of thousands of calls per speed test).
     * Previous builds emitted two IOLog records for every interrupt, which
     * serialized the networking path through the kernel log.  Perf counters
     * already track IRQ volume, so keep the data without logging per IRQ. */
...
bool RTW88IEEE80211::dataTxBatchingActive() const
{
    /* Defer doorbells only after the peer accepted our BlockAck agreement.
     * Until then every frame is kicked immediately, so association, EAPOL,
     * ARP and DHCP keep the reliable exp36 bootstrap path. */
    return _txBaActive && htAllowed();
}

void RTW88IEEE80211::kickDataTx()
{
    if (_rtwdev)
        rtw88_macos_tx_kick(_rtwdev);
}
    ...
    if (ieee80211_is_mgmt(fc)) {
        ...
        if (ieee80211_has_protected(fc)) {
            if (rxs->flag & RX_FLAG_DECRYPTED) {
                /* rtw88 advertises RX_INCLUDES_FCS. mac80211 normally removes
                 * these four bytes, but this port delivers directly. */
                uint16_t hlen = ieee80211_get_hdrlen_from_skb(skb);
                if (skb->len >= (uint32_t)hlen + 8 + 4)
                    skb_trim(skb, skb->len - 4);
                if (_parent)
                    _parent->setProperty("RTW88HardwareCcmpRxOK", 1, 64);
            } else if (!decryptRxCcmp(skb)) {
                kfree_skb(skb);
                return;
            }
        }
...
bool RTW88IEEE80211::acceptRxCcmpPn(bool group, uint8_t tid, uint64_t pn)
{
    tid &= 0x0f;
    uint16_t bit = (uint16_t)(1u << tid);
    uint16_t *valid = group ? &_ccmpRxValidGtk : &_ccmpRxValidPtk;
    uint64_t *highest = group ? _ccmpRxHighestGtk : _ccmpRxHighestPtk;
    uint64_t *bitmap = group ? _ccmpRxBitmapGtk : _ccmpRxBitmapPtk;
    ...
    if (pn > highest[tid]) {
        uint64_t delta = pn - highest[tid];
        bitmap[tid] = delta >= 64 ? 1 : ((bitmap[tid] << delta) | 1);
        highest[tid] = pn;
        return true;
    }
    uint64_t delta = highest[tid] - pn;
    if (delta >= 64 || (bitmap[tid] & (1ULL << delta)))
        return false;
    bitmap[tid] |= 1ULL << delta;
    return true;
}

bool RTW88IEEE80211::decryptRxCcmp(struct sk_buff *skb)
{
    if (!skb || skb->len < 24 + 8 + 8)
        return false;
    struct ieee80211_hdr *hdr = (struct ieee80211_hdr *)skb->data;
    uint16_t hlen = ieee80211_get_hdrlen_from_skb(skb);
    if (hlen < 24 || skb->len < (uint32_t)hlen + 16)
        return false;

    uint8_t *ccmp = skb->data + hlen;
    if (!(ccmp[3] & 0x20)) /* ExtIV is mandatory for CCMP */
        return false;
    uint8_t keyidx = (uint8_t)((ccmp[3] >> 6) & 0x03);
    bool group = is_multicast_ether_addr(hdr->addr1);
    struct ieee80211_key_conf *key = group ? _gtkConf : _ptkConf;
    if (!key || key->cipher != WLAN_CIPHER_SUITE_CCMP || key->keylen != 16)
        return false;
    if (group && (uint8_t)key->keyidx != keyidx)
        return false;

    uint8_t tid = 0;
    if (ieee80211_is_data_qos(hdr->frame_control))
        tid = (uint8_t)(skb->data[hlen - 2] & IEEE80211_QOS_CTL_TID_MASK);

    uint8_t aad[24] = {};
    aad[0] = skb->data[0] & 0x8f;
    aad[1] = skb->data[1] & 0xc7;
    memcpy(aad + 2, skb->data + 4, 18);
    aad[20] = skb->data[22] & 0x0f;
    aad[21] = 0;
    uint16_t aad_len = 22;
    if (ieee80211_is_data_qos(hdr->frame_control)) {
        aad[22] = tid;
    ...
    uint8_t nonce[13] = {};
    nonce[0] = tid;
    memcpy(nonce + 1, hdr->addr2, 6); /* transmitter address = AP */
    nonce[7] = ccmp[7]; nonce[8] = ccmp[6];
    nonce[9] = ccmp[5]; nonce[10] = ccmp[4];
    nonce[11] = ccmp[1]; nonce[12] = ccmp[0];
    ...
    uint8_t *cipher = ccmp + 8;
    bool ok = false;
    uint16_t plain_len = 0;
    /* RCR requests an appended FCS on this port.  Accept both descriptor
     * layouts because firmware revisions differ on whether those four bytes
     * are included in pkt_len. */
    static const uint8_t trailers[2] = { 12, 8 }; /* MIC+FCS, then MIC only */
    for (unsigned int i = 0; i < 2; i++) {
        uint32_t overhead = (uint32_t)hlen + 8 + trailers[i];
        if (skb->len <= overhead)
            continue;
        uint32_t candidate = skb->len - overhead;
        if (candidate > 0xffff)
            continue;
        const uint8_t *mic = cipher + candidate;
        if (ccmp_decrypt_80211(key->key, nonce, aad, aad_len, cipher,
                               (uint16_t)candidate, mic)) {
            plain_len = (uint16_t)candidate;
            ok = true;
            break;
        }
    }
    if (!ok || !acceptRxCcmpPn(group, tid, pn)) {
        _rxSoftwareCcmpFailed++;
        if (_rxSoftwareCcmpFailed == 1 && _parent)
            _parent->setProperty("RTW88SoftwareCcmpRxFailed", 1, 64);
        return false;
    }

    memmove(skb->data + hlen, cipher, plain_len);
    skb_trim(skb, (uint32_t)hlen + plain_len);
    hdr = (struct ieee80211_hdr *)skb->data;
    hdr->frame_control = cpu_to_le16(
        (uint16_t)(le16_to_cpu(hdr->frame_control) & ~IEEE80211_FCTL_PROTECTED));
    _rxSoftwareCcmpOk++;
    _rxDecryptedFrames++;
    if (_rxSoftwareCcmpOk == 1 && _parent)
        _parent->setProperty("RTW88SoftwareCcmpRxOK", 1, 64);
    return true;
}
...
void RTW88IEEE80211::processRxMgmt(struct sk_buff *skb)
{
    ...
    case 0x0050: /* probe response */
        /* exp10 watchdog: count target-AP beacons/probe responses while an
         * association exists (or is being established).  This is only used
         * after at least one target beacon has actually been observed. */
        if (skb->len >= sizeof(struct ieee80211_hdr_3addr)) {
            struct ieee80211_hdr_3addr *h3 =
                (struct ieee80211_hdr_3addr *)skb->data;
            bool target = memcmp(h3->addr3, _targetBSS.bssid, 6) == 0 ||
                          memcmp(h3->addr2, _targetBSS.bssid, 6) == 0;
            bool tracking = (_state == RTW88_STATE_AUTHENTICATING ||
        ...
        if (_state == RTW88_STATE_AUTHENTICATING) {
            ...
            if (body_len >= 6) {
                ...
                if (status == 0) {
                    ...
                    IOLog("rtw88: auth failed status=%u, scheduling recovery\n", status);
                    _timer->cancelTimeout();
                    _state = RTW88_STATE_IDLE;
                    scheduleReconnect("auth rejected");
                }
            } else {
    ...
    case 0x00C0: /* deauth */
        if (_state == RTW88_STATE_CONNECTED ||
            ...
            uint16_t reason = 0;
            {
                ...
                reason = (skb->len >= sizeof(*h3) + 2) ?
                         (uint16_t)(rb[0] | (rb[1] << 8)) : 0;
                IOLog("rtw88: %s from AP, reason=%u — auto-recovering\n",
                      (stype == 0x00C0) ? "deauth" : "disassoc", reason);
            }
            handleLinkLoss((stype == 0x00C0) ? "AP deauth" : "AP disassoc",
                           false, reason);
        }
        kfree_skb(skb);
        break;
...
void RTW88IEEE80211::processScanResult(struct sk_buff *skb)
{
    ...
    /* Parse the fixed beacon/probe-response fields before walking the IEs. */
    const uint8_t *body = skb->data + sizeof(struct ieee80211_hdr_3addr);
    const uint8_t *end  = skb->data + skb->len;
    uint16_t beaconInterval = (uint16_t)body[8] |
                              ((uint16_t)body[9] << 8);
    uint16_t capabilities = (uint16_t)body[10] |
                            ((uint16_t)body[11] << 8);
    ...
    bss->beacon_interval = beaconInterval;
    ...
    while (body + 2 <= end) {
        ...
        if (id == WLAN_EID_SSID && len > 0 && len <= 32) {
            ...
            uint8_t room = (uint8_t)(sizeof(bss->rates) - bss->nrates);
            uint8_t copyRates = len < room ? len : room;
            if (copyRates) {
                memcpy(bss->rates + bss->nrates, body + 2, copyRates);
                bss->nrates += copyRates;
            }
...
void RTW88IEEE80211::processRxData(struct sk_buff *skb)
{
    ...
    /* A QoS MPDU that belongs to an accepted RX BlockAck agreement must pass
     * through the per-TID reorder window.  rxReorderInput() deliberately falls
     * through to immediate delivery when no agreement is active, preserving
     * the exp36 DHCP bootstrap while allowing full downlink A-MPDU afterwards. */
    struct ieee80211_hdr *hdr = (struct ieee80211_hdr *)skb->data;
    if (ieee80211_is_data_qos(hdr->frame_control)) {
        uint16_t hdrlen = ieee80211_get_hdrlen_from_skb(skb);
        if (skb->len >= hdrlen && hdrlen >= 2) {
            uint8_t tid = skb->data[hdrlen - 2] & IEEE80211_QOS_CTL_TID_MASK;
            if (tid < kRxBaNumTid) {
                uint16_t sn = (uint16_t)((le16_to_cpu(hdr->seq_ctrl) >> 4) &
                                         0x0fff);
                rxReorderInput(tid, skb, sn);
                return;
            }
        }
    }
...
void RTW88IEEE80211::deliverDataFrame(struct sk_buff *skb)
{
    ...
    if (llc[0] == 0xAA && llc[1] == 0xAA && llc[2] == 0x03) {
        ...
        if (_parent)
            _parent->setProperty("RTW88LastRxInvalidLLC", 1, 64);
        kfree_skb(skb);
        return;
    }
                                     ...
                                     const uint8_t *payload, uint32_t paylen)
{
    ...
    _rxEthernetFrames++;
    if (_rxEthernetFrames == 1) {
        _parent->setProperty("RTW88RxEthernetFrames", 1, 64);
        _parent->setProperty("RTW88LastRxEtherType", ethertype, 32);
    }
...
{
    /* exp4: sample firmware TX reports (requested on 1/64 data frames).
     * Non-sampled frames still arrive here after normal PCI completion, but
     * only REQ_TX_STATUS frames carry a real firmware ACK result. */
    if (!skb) return;
    struct ieee80211_tx_info *info = IEEE80211_SKB_CB(skb);
    if (info->flags & IEEE80211_TX_CTL_REQ_TX_STATUS) {
        if (info->flags & IEEE80211_TX_STAT_ACK) {
            _txReportAcked++;
            if (_txReportAcked == 1 && _parent)
                _parent->setProperty("RTW88ProtectedTxAcked", 1, 64);
        } else {
            _txReportFailed++;
            if (_txReportFailed == 1 && _parent)
                _parent->setProperty("RTW88ProtectedTxFailed", 1, 64);
        }
    }
}
 ...
 * (from its band caps).  Without this the link is pinned to 20 MHz.  At
 * 80 MHz, the 1x1 RTL8821CE can negotiate up to a 433 Mbps PHY rate.
...
void RTW88IEEE80211::setConnectedChandef(struct ieee80211_channel *chan)
{
    ...
    if (_parent) {
        _parent->setProperty("RTW88TargetIESLength", ielen, 32);
        _parent->setProperty("RTW88HTSecondaryOffset", sco, 32);
        _parent->setProperty("RTW88HT40Allowed", htStaWidth ? 1 : 0, 32);
        _parent->setProperty("RTW88VHTOperationWidth", vhtWidth, 32);
        _parent->setProperty("RTW88VHTCenterSegment0", vhtSeg0, 32);
        _parent->setProperty("RTW88Chip80Capable", chip80 ? 1 : 0, 32);
    }
    ...
    if (_parent)
        _parent->setProperty("RTW88ConnectedChannelWidth", _connChanWidth, 32);
...
void RTW88IEEE80211::scanDone(bool aborted)
{
    ...
    if (_state == RTW88_STATE_SCANNING) {
        ...
        _manualScanOnHomeChannel = false;
        if (_state == RTW88_STATE_CONNECTED)
            armConnectedWatchdog();
    }
#ifdef RTW88_AIRPORT
    if (_parent)
        _parent->notifyAirportScanDone(aborted);
#endif
}

IOReturn RTW88IEEE80211::cmdScan()
{
    /* Background scans are valid while associated.  The implementation below
     * already preserves CONNECTED in _scanReturnState and restores the home
     * channel; rejecting that state made every post-association CoreWLAN scan
     * fail with EBUSY and caused wifiagent to retry continuously. */
...
{
    return cmdConnectInternal(ssid, password, nullptr);
}
                                            ...
                                            const uint8_t pmk[32])
{
    ...
    return cmdConnectInternal(ssid, nullptr, pmk);
}

IOReturn RTW88IEEE80211::cmdConnectInternal(const char *ssid,
                                             const char *password,
                                             const uint8_t *pmk)
{
    ...
    /* exp10: remember this as an intentional connection so an AP-side
     * deauth/disassoc can be healed without requiring a radio toggle. */
    _autoReconnectEnabled = true;
    _reconnectInProgress = false;
    _reconnectNeedsRadioReset = false;
    _radioRecoveryUsed = false;
    _reconnectAttemptStreak = 0;
    _targetBeaconCount = 0;
    _lastTargetBeaconCount = 0;
    _beaconMissWindows = 0;
    _targetBeaconEverSeen = false;
    _lastApDisconnectReason = 0;
...
{
    RTW88IEEE80211 *obj = (RTW88IEEE80211 *)self;
    if (obj) obj->runConnectWork();
}

void RTW88IEEE80211::runConnectWork()
{
    /* A delayed reconnect may still be queued when the user explicitly
     * disconnects/powers off.  Bail out before touching RF in that case. */
    if (!_autoReconnectEnabled || !_powered || !_hw || !_vif ||
        _state != RTW88_STATE_AUTHENTICATING)
        return;

    if (_reconnectNeedsRadioReset) {
        _reconnectNeedsRadioReset = false;
        IOLog("rtw88: exp10 reconnect escalation — soft radio restart\n");
        powerOff();
        IOSleep(120);
        if (powerOn() != kIOReturnSuccess) {
            IOLog("rtw88: exp10 soft radio restart failed\n");
            _state = RTW88_STATE_IDLE;
            return;
        }
        _radioRecoveryCount++;
...
void RTW88IEEE80211::scheduleReconnect(const char *reason)
{
    if (!_autoReconnectEnabled || !_powered || !_connectTC ||
        _targetBSS.ssid_len == 0)
        return;

    _timer->cancelTimeout();
    _reconnectInProgress = true;
    _reconnectAttemptStreak++;
    _autoReconnectAttempts++;

    /* Reproduce the user's known-good off/on recovery automatically only
     * after two lightweight reassociation attempts.  Do it once per outage
     * so a bad AP cannot create a power-cycle loop. */
    if (_reconnectAttemptStreak >= 3 && !_radioRecoveryUsed) {
        _reconnectNeedsRadioReset = true;
        _radioRecoveryUsed = true;
    }

    uint32_t delayMs = 400U * _reconnectAttemptStreak;
    if (delayMs > 2000U) delayMs = 2000U;
    _state = RTW88_STATE_AUTHENTICATING;

    thread_call_cancel(_connectTC);
    uint64_t deadline = 0;
    clock_interval_to_deadline(delayMs, kMillisecondScale, &deadline);
    thread_call_enter_delayed(_connectTC, deadline);
    IOLog("rtw88: exp10 auto-reconnect #%u in %u ms (%s)\n",
          _reconnectAttemptStreak, delayMs, reason ? reason : "link loss");
}

void RTW88IEEE80211::handleLinkLoss(const char *reason, bool watchdog,
                                    uint16_t apReason)
{
    bool reconnect = _autoReconnectEnabled && _powered;
    _linkLossEvents++;
    if (watchdog) _beaconWatchdogRecoveries++;
    if (apReason) _lastApDisconnectReason = apReason;

    if (_state == RTW88_STATE_SCANNING)
        _manualScanAbort = true;
    _timer->cancelTimeout();

    /* Tear down ALL association state before reusing the vif.  The old code
     * only cleared keys/BA on remote deauth and left vif->cfg.assoc and the
     * rtw88 STA registered, which is why a later connect often needed a full
     * Wi-Fi off/on cycle. */
    clearKeys();
    if (_vif) {
        struct ieee80211_bss_conf *bss = &_vif->bss_conf;
        ...
        _vif->cfg.aid = 0;
        if (_powered && _hw && _hw->ops && _hw->ops->bss_info_changed)
    ...
    if (_parent) {
        ...
        _parent->resetTxFlowForReconnect();
#ifdef RTW88_AIRPORT
        _parent->notifyAirportLinkDown(apReason);
#endif
    }

    IOLog("rtw88: exp10 link recovery: %s ap-reason=%u reconnect=%d\n",
          reason ? reason : "unknown", apReason, reconnect ? 1 : 0);
    if (reconnect)
...
void RTW88IEEE80211::markConnected()
{
    bool recovered = _reconnectInProgress;
    _state = RTW88_STATE_CONNECTED;
    _timer->cancelTimeout();

    if (recovered)
        _autoReconnectSuccesses++;
    _reconnectInProgress = false;
    _reconnectNeedsRadioReset = false;
    _reconnectAttemptStreak = 0;
    _radioRecoveryUsed = false;
    _beaconMissWindows = 0;
    _lastTargetBeaconCount = _targetBeaconCount;
#ifdef RTW88_AIRPORT
    if (_parent)
        _parent->notifyAirportLinkUp();
#else
    if (_parent)
    ...
    startTxAggregation();
    armConnectedWatchdog();
    IOLog("rtw88: exp10 link active%s\n", recovered ? " (auto-recovered)" : "");
}

void RTW88IEEE80211::doAuthenticate()
{
    if (!_hw || !_vif || !_powered ||
        _state != RTW88_STATE_AUTHENTICATING) return;
...
void RTW88IEEE80211::processAssocResponse(struct sk_buff *skb)
{
    ...
    /* exp10: consume the association status/AID BEFORE freeing the skb.
     * exp8 and earlier kept a pointer into skb->data and dereferenced it after
     * kfree_skb(), a use-after-free that made association/reassociation
     * nondeterministic under memory pressure. */
    if (bodylen < 6) {
        kfree_skb(skb);
        IOLog("rtw88: assoc-resp too short\n");
        _state = RTW88_STATE_IDLE;
        scheduleReconnect("short assoc response");
        return;
    }
    ...
    if (status != 0) {
        ...
        _state = RTW88_STATE_IDLE;
        scheduleReconnect("association rejected");
        return;
    }
    ...
    if (_sta == nullptr && _hw->ops && _hw->ops->sta_add) {
        ...
        if (_sta) {
            ...
            int sta_ret = _hw->ops->sta_add(_hw, _vif, _sta);
            if (sta_ret == 0) {
                /* exp5: mac80211 station table replacement for this 1-STA port.
                 * rtw88 RX and watchdog paths use find/iterate station helpers
                 * to maintain avg_rssi and feed rate adaptation. */
    ...
    if (!_sta) {
        _state = RTW88_STATE_IDLE;
        scheduleReconnect("STA allocation/add failed");
        return;
    }
...
void RTW88IEEE80211::doDisconnect()
{
    ...
    if (_parent) {
        ...
        _parent->resetTxFlowForReconnect();
#ifdef RTW88_AIRPORT
        _parent->notifyAirportLinkDown(WLAN_REASON_DEAUTH_LEAVING);
#endif
    }
}
...
void RTW88IEEE80211::handleEAPOL(const uint8_t *data, uint32_t len)
{
    ...
    const uint8_t *rx_replay = data + 9;

    IOLog("rtw88: EAPOL key_info=0x%04x key_data_len=%u M1=%d M3=%d\n",
          key_info, key_data_len, is_m1, is_m3);

    if (is_m1) {
        if (_eapolReplayValid) {
            int order = rtw88_replay_cmp(rx_replay, _replayCtr);
            if (order < 0) {
                IOLog("rtw88: exp22 dropped stale EAPOL M1\n");
                return;
            }
            if (order == 0) {
                if (!_eapolM1Seen ||
                    !rtw88_consttime_equal(data + 17, _anonce, sizeof(_anonce))) {
                    IOLog("rtw88: exp22 rejected M1 replay with changed ANonce\n");
                    return;
                }
        ...
        memcpy(_replayCtr, rx_replay, 8);
        _eapolReplayValid = true;
        _eapolM1Seen = true;
        _ptkInstalledThisHandshake = false;
        sendEAPOLKey(2, _replayCtr, false, false, true);
        uint64_t d;
        ...
        if (!_eapolM1Seen || !_eapolReplayValid) {
            IOLog("rtw88: exp22 dropped M3 without active M1\n");
            return;
        }

        int order = rtw88_replay_cmp(rx_replay, _replayCtr);
        if (order < 0) {
            IOLog("rtw88: exp22 dropped stale EAPOL M3\n");
            return;
        }
        ...
        /* A valid retransmitted M3 is acknowledged, but the already-active
         * PTK/GTK is never reinstalled. Reinstalling would reset CCMP TX PN. */
        if (_ptkInstalledThisHandshake) {
            if (order > 0)
                memcpy(_replayCtr, rx_replay, 8);
            sendEAPOLKey(4, rx_replay, false, false, true);
            IOLog("rtw88: exp22 blocked EAPOL M3 key reinstallation; M4 resent\n");
            return;
        }
        ...
        _ptkInstalledThisHandshake = true;
...
void RTW88IEEE80211::handleBackAction(const uint8_t *b, uint32_t len)
{
    ...
    case WLAN_ACTION_ADDBA_REQ: {
        ...
        if (tid >= kRxBaNumTid) {
            /* Status 37 = request declined. */
            sendAddbaResponse(tid, dialog, req_param, ba_to, 37);
            IOLog("rtw88: RX ADDBA request unsupported tid=%u declined\n", tid);
            break;
        }

        rxBaSetup(tid, ssn, bufsz);
        sendAddbaResponse(tid, dialog, req_param, ba_to, 0);
        if (_parent) {
            _parent->setProperty("RTW88RxBlockAckActive", 1, 32);
            _parent->setProperty("RTW88RxBlockAckLastTID", tid, 32);
        }
        IOLog("rtw88: RX ADDBA accepted tid=%u ssn=%u window=%u\n",
              tid, ssn, bufsz ? bufsz : kRxBaMaxBuf);
        break;
    }
    case WLAN_ACTION_ADDBA_RESP: {
        ...
        if (status == 0 && tid == _baTid) {
            _txBaActive = true;
            if (_parent)
                _parent->setProperty("RTW88TxBlockAckActive", 1, 32);
...
bool RTW88IEEE80211::txNullFunc(bool powerSave)
{
    ...
    /* exp4: ask firmware for a real ACK result on only 1/64 data frames.
     * This is sparse enough to avoid turning the C2H report path into a new
     * bottleneck, while revealing whether low throughput is really on-air
     * loss/retry rather than the macOS delivery path. */
    _txDataFrames++;
    if ((_txDataFrames & 63ULL) == 0) {
...
bool RTW88IEEE80211::txDataFrame(mbuf_t m)
{
    ...
    /* EAPOL remains legacy data during the 4-way handshake.  Once keys are
     * installed, normal Ethernet traffic uses the WME/QoS format negotiated
     * in the association request, enabling HT/VHT rates and BlockAck. */
    bool qos = htAllowed() && ethertype != ETH_P_PAE;
    bool unicast = (eh[0] & 0x01) == 0;
    uint32_t hlen = qos ? 26 : 24;
    /* mac80211 supplies the CCMP IV; RTL8821CE encrypts the MSDU and appends
     * the MIC when the pairwise hardware key is attached to tx_info. */
    ...
    info->flags = IEEE80211_TX_CTL_FIRST_FRAGMENT;
    _txDataFrames++;
    if (protected_frame && _txReportRequested < 8) {
        info->flags |= IEEE80211_TX_CTL_REQ_TX_STATUS;
        _txReportRequested++;
    }
    ...
    info->control.vif = _vif;
    info->control.sta = _sta;
    if (qos && unicast && protected_frame && _txBaActive)
        info->flags |= IEEE80211_TX_CTL_AMPDU;
    if (protected_frame) {
        info->control.hw_key = _ptkConf;
        if (_parent)
            _parent->setProperty("RTW88HardwareCcmpTx", 1, 64);
    }
...
{
    /* macOS integration: the lightweight RSSI/RA watchdog tick runs on this
     * object's IOWorkLoop, serialized with state changes and IOUserClient
     * calls.  The generic Linux RF watchdog remains disabled. */
    if (_rtwdev && _state == RTW88_STATE_CONNECTED)
        rtw88_macos_watchdog_tick(_rtwdev);
    ...
    case RTW88_STATE_AUTHENTICATING:
        IOLog("rtw88: auth timeout — scheduling clean reconnect\n");
        _state = RTW88_STATE_IDLE;
        scheduleReconnect("auth timeout");
        break;

    case RTW88_STATE_ASSOCIATING:
        IOLog("rtw88: assoc timeout — scheduling clean reconnect\n");
        _state = RTW88_STATE_IDLE;
        scheduleReconnect("assoc timeout");
        break;

    case RTW88_STATE_HANDSHAKING:
        IOLog("rtw88: 4-way handshake timeout — recovering link\n");
        handleLinkLoss("4-way handshake timeout", false, 0);
        break;

    case RTW88_STATE_CONNECTED:
        /* Only enforce beacon liveness if this driver instance has actually
         * observed target beacons.  Some firmware filter modes may hide all
         * beacons; in that case remote deauth + state cleanup still provides
         * recovery without a false watchdog disconnect. */
        if (_targetBeaconEverSeen) {
            if (_targetBeaconCount == _lastTargetBeaconCount)
                _beaconMissWindows++;
            else
                _beaconMissWindows = 0;
            _lastTargetBeaconCount = _targetBeaconCount;

            if (_beaconMissWindows >= 3) {
                IOLog("rtw88: exp10 beacon watchdog: target silent for ~12s\n");
                handleLinkLoss("beacon watchdog", true, 0);
                break;
            }
        }
        armConnectedWatchdog();
...
IOReturn RTW88IEEE80211::cmdGetState(struct RTW88StateResult *result)
{
    ...
    /* Snapshot-only path: do not touch _rtwdev, _hw or their callbacks here.
     * They are freed by rtw_pci_remove() during teardown and status requests
     * are not serialized with that lifetime.  Traffic counters are
     * diagnostic-only, so return the zero-initialized values rather than
     * risking the machine for live counters. */
    result->fw_version = _fwVersion;
    result->fw_sub_version = _fwSubVersion;
    strlcpy(result->chip_name,
            _chipName[0] ? _chipName : "Uninitialized",
            sizeof(result->chip_name));
    result->scan_offload_supported = _scanOffloadSupported ? 1 : 0;
...
IOReturn RTW88IEEE80211::cmdSetTxRate(uint8_t rate)
{
    if (!_hw || !_rtwdev) return kIOReturnNotReady;
    /* 0xff = firmware RA.  Forced descriptor rates are diagnostic only. */
    if (rate != 0xff && (rate < 0x2c || rate > 0x35))
        return kIOReturnBadArgument;
    rtw88_set_fix_rate(_hw, rate);
    return kIOReturnSuccess;
}

IOReturn RTW88IEEE80211::cmdGetPerf(struct RTW88PerfResult *result)
{
    if (!result) return kIOReturnBadArgument;
    ...
    result->rssi = _rssi;
    result->channel = _targetBSS.channel;
    result->channel_width_mhz = _connChanWidth;
    rtw88_get_stats(_rtwdev, &result->tx_byte_count, &result->rx_byte_count);
    result->be_tx_free = rtw88_be_tx_avail();
    result->tx_ba_active = _txBaActive ? 1 : 0;
    result->powered = _powered ? 1 : 0;

    uint8_t rx_active = 0;
    if (_rxBaLock) IOLockLock(_rxBaLock);
    for (uint8_t tid = 0; tid < kRxBaNumTid; tid++)
        if (_rxBa[tid] && _rxBa[tid]->active) rx_active++;
    if (_rxBaLock) IOLockUnlock(_rxBaLock);
    result->rx_ba_active_count = rx_active;

    /* exp4: expose the firmware's actual selected TX/RX descriptor rates and
     * the STA RA state. Width=80 alone only proves channel configuration; it
     * does not prove that firmware is transmitting at a VHT MCS rate. */
    struct rtw88_link_diag ld = {};
    rtw88_get_link_diag(_hw, _sta, &ld);
    result->fw_tx_desc_rate = ld.fw_tx_desc_rate;
    result->fw_rx_desc_rate = ld.fw_rx_desc_rate;
    result->sta_vht = ld.sta_vht;
    result->sta_sgi = ld.sta_sgi;
    result->sta_bw_mode = ld.sta_bw_mode;
    result->sta_rate_id = ld.sta_rate_id;
    result->ra_mask = ld.ra_mask;
    result->ra_bit_rate = ld.ra_bit_rate;
    result->ra_mcs = ld.ra_mcs;
    result->ra_nss = ld.ra_nss;
    result->ra_bw = ld.ra_bw;
    result->ra_flags = ld.ra_flags;
    result->sta_avg_rssi = ld.sta_avg_rssi;
    result->sta_rssi_level = ld.sta_rssi_level;
    result->ra_calibrated_rssi = ld.ra_calibrated_rssi;
    result->ra_fw_rssi = ld.ra_fw_rssi;
    result->dm_min_rssi = ld.dm_min_rssi;
    result->sta_bridge_registered = ld.sta_bridge_registered;
    result->watchdog_count = ld.watchdog_count;
    result->sta_iter_calls = ld.sta_iter_calls;
    result->sta_find_hits = ld.sta_find_hits;
    result->txpi_rate = ld.txpi_rate;
    result->txpi_rate_id = ld.txpi_rate_id;
    result->txpi_bw = ld.txpi_bw;
    result->txpi_short_gi = ld.txpi_short_gi;
    result->txpi_use_rate = ld.txpi_use_rate;
    result->txpi_dis_rate_fallback = ld.txpi_dis_rate_fallback;
    result->txpi_ampdu_en = ld.txpi_ampdu_en;
    result->fix_rate = ld.fix_rate;
    result->txpi_samples = ld.txpi_samples;
    result->link_loss_events = _linkLossEvents;
    result->auto_reconnect_attempts = _autoReconnectAttempts;
    result->auto_reconnect_successes = _autoReconnectSuccesses;
    result->radio_recovery_count = _radioRecoveryCount;
    result->beacon_watchdog_recoveries = _beaconWatchdogRecoveries;
    result->irq_inline_bottom_halves = rtw88_get_inline_irq_bh_count();
    result->reconnect_attempt_streak = _reconnectAttemptStreak;
    result->last_ap_disconnect_reason = _lastApDisconnectReason;
    result->beacon_miss_windows = _beaconMissWindows;
    result->beacon_watchdog_active = _targetBeaconEverSeen ? 1 : 0;
    result->tx_report_requested = _txReportRequested;
    result->tx_report_acked = _txReportAcked;
    result->tx_report_failed = _txReportFailed;
    result->rx_reorder_inputs = _rxReorderInputs;
    result->rx_reorder_stale = _rxReorderStale;
    result->rx_reorder_duplicates = _rxReorderDuplicates;
    result->rx_reorder_ahead = _rxReorderAhead;
    result->rx_reorder_timeout_flushes = _rxReorderTimeoutFlush;
    result->rx_reorder_holes_skipped = _rxReorderHolesSkipped;
    rtw88_get_sched_diag(&result->irq_count, &result->napi_polls,
                         &result->napi_work, &result->napi_budget_hits);
    rtw88_get_napi_coalesce_diag(&result->napi_schedule_calls,
                                  &result->napi_coalesced_calls,
                                  &result->napi_delayed_calls,
                                  &result->napi_flush_callbacks);

    if (_parent) {
        result->dma_tx_maps = _parent->dmaTxMaps();
        result->dma_rx_maps = _parent->dmaRxMaps();
        result->dma_tx_bytes = _parent->dmaTxBytes();
        result->dma_rx_bytes = _parent->dmaRxBytes();
        result->dma_rx_copy_bytes = _parent->dmaRxCopyBytes();
        result->tx_stall_events = _parent->txStallEvents();
        result->tx_pool_hits = _parent->txPoolHits();
        result->tx_pool_fallbacks = _parent->txPoolFallbacks();
        result->rx_queued_packets = _parent->rxQueuedPackets();
        result->rx_flushes = _parent->rxFlushes();
        result->tx_pool_in_use = _parent->txPoolInUse();
        result->tx_pool_peak = _parent->txPoolPeak();
        result->tx_batch_packets = _parent->txBatchPackets();
        result->tx_batch_kicks = _parent->txBatchKicks();
        result->tx_batch_timer_kicks = _parent->txBatchTimerKicks();
        result->tx_batch_immediate_kicks = _parent->txBatchImmediateKicks();
        result->tx_batch_pending = _parent->txBatchPending();
        result->tx_batch_max_pending = _parent->txBatchMaxPending();
    }
...
IOReturn RTW88IEEE80211::cmdGetAirportBSSList(RTW88BSS *records,
                                               uint32_t *count)
{
    if (!records || !count)
        return kIOReturnBadArgument;
    ...
    IOLockLock(_bssLock);
    for (RTW88BSS *b = _bssList; b && written < capacity; b = b->next) {
        memcpy(&records[written], b, sizeof(*b));
        records[written].next = nullptr;
        written++;
    }
    IOLockUnlock(_bssLock);
