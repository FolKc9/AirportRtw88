...
/* exp10 defaults to WARN for normal operation.  exp8/exp9 ran at DEBUG and
 * pushed every core debug/info message through IOLog.  The CLI can still
 * temporarily raise this with `rtw88ctl debug 2|3` when troubleshooting. */
int rtw88_log_level = KERN_WARN;
...
/* macOS-safe default: keep the full Linux RF watchdog disabled.  The
 * lightweight RSSI/RA feed remains active and avoids the concurrent RF work
 * that caused kernel restarts when the IOUserClient was accessed. */
/* EXPERIMENTAL: full Linux RF watchdog requested by the user. */
bool rtw88_disable_watchdog_work = true;

/* The full Linux watchdog remains disabled.  Its lightweight RSSI/RA portion
 * is driven by RTW88IEEE80211::onTimer on the owning IOWorkLoop instead. */
...
/* exp4 scheduler diagnostics: approximate counters, read only by perf. */
static volatile u64 g_diag_irq_count = 0;
static volatile u64 g_diag_inline_irq_bh = 0;
static volatile u64 g_diag_napi_polls = 0;
static volatile u64 g_diag_napi_work = 0;
static volatile u64 g_diag_napi_budget_hits = 0;
/* exp11: asynchronous NAPI moderation diagnostics. */
static volatile u64 g_diag_napi_schedule_calls = 0;
static volatile u64 g_diag_napi_coalesced_calls = 0;
static volatile u64 g_diag_napi_delayed_calls = 0;
static volatile u64 g_diag_napi_flush_callbacks = 0;

void rtw88_get_sched_diag(u64 *irq_count, u64 *napi_polls, u64 *napi_work,
                          u64 *napi_budget_hits)
{
    if (irq_count) *irq_count = g_diag_irq_count;
    if (napi_polls) *napi_polls = g_diag_napi_polls;
    if (napi_work) *napi_work = g_diag_napi_work;
    if (napi_budget_hits) *napi_budget_hits = g_diag_napi_budget_hits;
}

void rtw88_get_napi_coalesce_diag(u64 *schedule_calls, u64 *coalesced_calls,
                                  u64 *delayed_calls, u64 *flush_callbacks)
{
    if (schedule_calls) *schedule_calls = g_diag_napi_schedule_calls;
    if (coalesced_calls) *coalesced_calls = g_diag_napi_coalesced_calls;
    if (delayed_calls) *delayed_calls = g_diag_napi_delayed_calls;
    if (flush_callbacks) *flush_callbacks = g_diag_napi_flush_callbacks;
}

u64 rtw88_get_inline_irq_bh_count(void) { return g_diag_inline_irq_bh; }
...
/* exp5: Feixiao currently supports one associated peer.  mac80211 normally
 * owns the station table; our shim must expose that peer to rtw88 instead. */
static struct ieee80211_sta *g_rtw88_sta = NULL;
static volatile u64 g_diag_sta_iter_calls = 0;
static volatile u64 g_diag_sta_find_hits = 0;
...
static void (*g_napi_flush_cb)(void) = NULL;
void rtw88_set_tx_resume_cb(void (*cb)(void)) { g_tx_resume_cb = cb; }
void rtw88_set_napi_flush_cb(void (*cb)(void)) { g_napi_flush_cb = cb; }
...
void rtw88_trigger_interrupt(void)
{
    ...
    if (g_irq_handler && g_irq_dev_id) {
        ...
        if (ret == IRQ_WAKE_THREAD) {
            /* exp10: IOInterruptEventSource has already moved us out of the
             * hardware interrupt context and onto the controller workloop.
             * The old port then paid for a second thread_call hop before the
             * rtw88 threaded IRQ could reclaim TX descriptors.  Under load
             * that delay showed up as persistent BE-ring backpressure.
             *
             * Run the threaded half here, on the already-deferred workloop.
             * NAPI itself remains deferred through its own thread_call, so we
             * do not poll RX while the PCI IRQ thread holds irq_lock. */
            if (g_irq_thread_fn) {
                g_irq_thread_fn(0, g_irq_dev_id);
                g_diag_inline_irq_bh++;
                if (g_tx_resume_cb)
                    g_tx_resume_cb();
            } else if (g_irq_thread_call) {
...
static void rtw88_napi_thread_wrapper(thread_call_param_t param0, thread_call_param_t param1)
{
    ...
    if (napi && napi->poll) {
        ...
        g_diag_napi_polls++;
        if (work > 0) {
            g_diag_napi_work += (u64)work;
            if (g_napi_flush_cb) {
                g_napi_flush_cb();
                g_diag_napi_flush_callbacks++;
            }
        }
        /* exp12 adaptive moderation: the exp11 trace averaged only 1.33 RX
         * descriptors/poll at 75 us. Increase the next visibility window when
         * polls are sparse; shorten it automatically once useful batching is
         * already happening. This changes processing cadence, not queue size. */
        napi->last_work = work;
        if (work <= 1)
            napi->coalesce_us = 120;
        else if (work <= 3)
            napi->coalesce_us = 100;
        else if (work <= 7)
            napi->coalesce_us = 110;
        else
            napi->coalesce_us = 60;

        if (work >= napi->weight) {
            g_diag_napi_budget_hits++;
            if (napi->thread_call)
...
{
    if (!napi || !napi->poll || !napi->thread_call) return;
    g_diag_napi_schedule_calls++;
    /* exp12 preserves the Linux NAPI scheduled-bit behavior restored by exp11;
     * original shim. Repeated IRQ-side schedule requests collapse into one. */
    if (napi->running) {
        g_diag_napi_coalesced_calls++;
        return;
    }
    napi->running = 1;

    /* exp12 non-blocking adaptive moderation. RX is already masked by rtw88
     * before napi_schedule(); delay is selected from the previous poll density
     * (60..220 us), never by busy-waiting. */
    uint64_t deadline = 0;
    unsigned int delay_us = napi->coalesce_us ? napi->coalesce_us : 170;
    clock_interval_to_deadline(delay_us, kMicrosecondScale, &deadline);
    thread_call_enter_delayed((thread_call_t)napi->thread_call, deadline);
    g_diag_napi_delayed_calls++;
}
...
{
    (void)hw;
    __sync_fetch_and_add(&g_diag_sta_iter_calls, 1);
                                          ...
                                          const u8 *addr)
{
    if (!g_rtw88_sta)
        return NULL;
    if (vif && g_rtw88_vif && vif != g_rtw88_vif)
        return NULL;
    if (addr && memcmp(g_rtw88_sta->addr, addr, ETH_ALEN) != 0)
        return NULL;
    __sync_fetch_and_add(&g_diag_sta_find_hits, 1);
    return g_rtw88_sta;
}
                                                    ...
                                                    const u8 *localaddr)
{
    ...
    if (addr && memcmp(g_rtw88_sta->addr, addr, ETH_ALEN) != 0)
        return NULL;
    if (localaddr && g_rtw88_vif &&
        memcmp(g_rtw88_vif->addr, localaddr, ETH_ALEN) != 0)
        return NULL;
    __sync_fetch_and_add(&g_diag_sta_find_hits, 1);
    return g_rtw88_sta;
}
...
static volatile u8 g_exp7_txpi_rate = 0;
static volatile u8 g_exp7_txpi_rate_id = 0;
static volatile u8 g_exp7_txpi_bw = 0;
static volatile u8 g_exp7_txpi_short_gi = 0;
static volatile u8 g_exp7_txpi_use_rate = 0;
static volatile u8 g_exp7_txpi_dis_fb = 0;
static volatile u8 g_exp7_txpi_ampdu = 0;
static volatile u64 g_exp7_txpi_samples = 0;

void rtw88_exp7_tx_diag_record(const struct rtw_tx_pkt_info *pkt)
{
    if (!pkt) return;
    g_exp7_txpi_rate = pkt->rate;
    g_exp7_txpi_rate_id = pkt->rate_id;
    g_exp7_txpi_bw = pkt->bw;
    g_exp7_txpi_short_gi = pkt->short_gi ? 1 : 0;
    g_exp7_txpi_use_rate = pkt->use_rate ? 1 : 0;
    g_exp7_txpi_dis_fb = pkt->dis_rate_fallback ? 1 : 0;
    g_exp7_txpi_ampdu = pkt->ampdu_en ? 1 : 0;
    g_exp7_txpi_samples++;
}
                         ...
                         struct rtw88_link_diag *out)
{
    ...
    struct rtw_dev *rtwdev = (struct rtw_dev *)hw->priv;
    out->fw_tx_desc_rate = rtwdev->dm_info.tx_rate;
    out->fw_rx_desc_rate = rtwdev->dm_info.curr_rx_rate;

    if (!sta || !sta->drv_priv) return;
    struct rtw_sta_info *si = (struct rtw_sta_info *)sta->drv_priv;
    out->sta_vht = si->vht_enable ? 1 : 0;
    out->sta_sgi = si->sgi_enable ? 1 : 0;
    out->sta_bw_mode = si->bw_mode;
    out->sta_rate_id = si->rate_id;
    out->ra_mask = si->ra_mask;
    out->ra_bit_rate = si->ra_report.bit_rate;
    out->ra_mcs = si->ra_report.txrate.mcs;
    out->ra_nss = si->ra_report.txrate.nss;
    out->ra_bw = si->ra_report.txrate.bw;
    out->ra_flags = (u8)si->ra_report.txrate.flags;
    out->sta_avg_rssi = ewma_rssi_read(&si->avg_rssi);
    out->sta_rssi_level = si->rssi_level;
    out->ra_calibrated_rssi = si->macos_ra_calibrated_rssi;
    out->ra_fw_rssi = si->macos_ra_fw_rssi;
    out->dm_min_rssi = rtwdev->dm_info.min_rssi;
    out->sta_bridge_registered = (g_rtw88_sta == sta) ? 1 : 0;
    out->watchdog_count = rtwdev->watch_dog_cnt;
    out->sta_iter_calls = g_diag_sta_iter_calls;
    out->sta_find_hits = g_diag_sta_find_hits;
    out->txpi_rate = g_exp7_txpi_rate;
    out->txpi_rate_id = g_exp7_txpi_rate_id;
    out->txpi_bw = g_exp7_txpi_bw;
    out->txpi_short_gi = g_exp7_txpi_short_gi;
    out->txpi_use_rate = g_exp7_txpi_use_rate;
    out->txpi_dis_rate_fallback = g_exp7_txpi_dis_fb;
    out->txpi_ampdu_en = g_exp7_txpi_ampdu;
    out->fix_rate = rtwdev->dm_info.fix_rate;
    out->txpi_samples = g_exp7_txpi_samples;
}
#include "reg.h"
                             ...
                             const uint8_t *bssid)
{
    ...
    /* Bootstrap protected unicast at the mandatory 6 Mbps OFDM rate.  The
     * association/EAPOL path works, but firmware RA has no successful protected
     * data sample yet and starts too aggressively; DHCP then never receives an
     * ACK/OFFER.  Once basic connectivity is proven this can be relaxed back to
     * firmware RA in a later throughput-oriented build. */
